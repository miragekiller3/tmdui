...
#define nsvg__roundf CFloat::Round
...
//      nsvg__addPathPoint(r, path->pts[0]*scale, path->pts[1]*scale, 0);
        nsvg__addPathPoint(r, path->pts[0].X()*scale, path->pts[0].Y()*scale, 0);
        for (i = 0; i < path->npts-1; i += 3) {
//          float* p = &path->pts[i*2];
            float* p = (float*)&path->pts[i];
...
//      nsvg__addPathPoint(r, path->pts[0]*scale, path->pts[1]*scale, NSVG_PT_CORNER);
        nsvg__addPathPoint(r, path->pts[0].X()*scale, path->pts[0].Y()*scale, NSVG_PT_CORNER);
        for (i = 0; i < path->npts-1; i += 3) {
//          float* p = &path->pts[i*2];
            float* p = (float*)&path->pts[i];
...
//  memcpy(cache->xform, grad->xform, sizeof(float)*6);
    (*(TM::CAffine*)cache->xform) = grad->xform;
